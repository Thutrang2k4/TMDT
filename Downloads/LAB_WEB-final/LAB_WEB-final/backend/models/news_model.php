...
function get_all_posts($category_id = null, $keyword = '') {
    ...
    $sql = "SELECT p.*, c.name as category_name 
            ...
            WHERE 1=1";
    if ($category_id !== null) $sql .= " AND p.category_id = " . intval($category_id);
    if (!empty($keyword)) {
...
function get_all_news($keyword = '') { return get_all_posts(null, $keyword); }
...
// --- PHẦN MỚI THÊM CHO COMMENT ---

// 1. Lấy comment theo ID bài viết (Chỉ lấy status = approved hoặc pending tùy logic, ở đây lấy approved)
...
function add_post_comment($post_id, $user_id, $content, $rating) {
    global $conn;
    // Mặc định là 'approved' để hiện ngay (hoặc 'pending' nếu muốn duyệt)
    $status = 'approved'; 
